...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        { // Drawing guide for trail
            float const distance = sqrt(pow(mouse.x - sf::Mouse::getPosition(window).x, 2) +
                                        pow(mouse.y - sf::Mouse::getPosition(window).y, 2));
            float const angle =
                bnw::getEquationAngle((sf::Vector2f)mouse, (sf::Vector2f)sf::Mouse::getPosition(window));
            ...
            if (distance < 200)
            {
                ...
                trail[0] = sf::Vertex(
                    sf::Vector2f(Ball::active_ball->getPos().x + x, Ball::active_ball->getPos().y + y), trail_color);
                trail[1] = sf::Vertex(
                    sf::Vector2f(Ball::active_ball->getPos().x - x, Ball::active_ball->getPos().y - y), trail_color);
                trail[2] = sf::Vertex(
                    sf::Vector2f(2 * (sf::Vector2i)Ball::active_ball->getPos() - sf::Mouse::getPosition(window)),
                    trail_color);

                Ball::active_ball->body.setFillColor(trail_color);
            }
            else
            { // Pushing velocity cap
                trail[0] = sf::Vertex(
                    sf::Vector2f(Ball::active_ball->getPos().x + x, Ball::active_ball->getPos().y + y), sf::Color::Red);
                trail[1] = sf::Vertex(
                    sf::Vector2f(Ball::active_ball->getPos().x - x, Ball::active_ball->getPos().y - y), sf::Color::Red);
                ...
                {
                    trail[2] = sf::Vertex(
                        sf::Vector2f(Ball::active_ball->getPos().x + tip_x, Ball::active_ball->getPos().y + tip_y),
                        sf::Color::Red);
                }
                else
                {
