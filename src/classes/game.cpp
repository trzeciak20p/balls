...
bool Game::calculateTrail()
{
    ...
    const float distance = std::sqrt(pow(mouse.x - sf::Mouse::getPosition(*m_window).x, 2) +
    ...
    if (distance < 200)
    {
        ...
        trail[0] =
            sf::Vertex(sf::Vector2f(active_ball->getPosition().x + x, active_ball->getPosition().y + y), trail_color);
        trail[1] =
            sf::Vertex(sf::Vector2f(active_ball->getPosition().x - x, active_ball->getPosition().y - y), trail_color);
        trail[2] =
            sf::Vertex(sf::Vector2f(2 * sf::Vector2i(active_ball->getPosition()) - sf::Mouse::getPosition(*m_window)),
                       trail_color);
    ...
    { // speed velocity cap
        trail[0]          = sf::Vertex(sf::Vector2f(active_ball->getPosition().x + x, active_ball->getPosition().y + y),
                                       sf::Color::Red);
        trail[1]          = sf::Vertex(sf::Vector2f(active_ball->getPosition().x - x, active_ball->getPosition().y - y),
                                       sf::Color::Red);
        ...
        {
            trail[2] =
                sf::Vertex(sf::Vector2f(active_ball->getPosition().x + tip_x, active_ball->getPosition().y + tip_y),
                           sf::Color::Red);
        }
        else
        {
